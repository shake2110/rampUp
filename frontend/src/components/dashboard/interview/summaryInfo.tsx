...
function SummaryInfo({ responses, interview }: SummaryProps) {
  ...
  const prepareTableData = (responses: Response[]): TableData[] => {
    return responses.map((response) => ({
      ...
      clarityScore: response.analytics?.clarity?.score ?? null,
      simplicityScore: response.analytics?.simplicity?.score ?? null,
      patienceScore: response.analytics?.patience?.score ?? null,
      warmthScore: response.analytics?.warmth?.score ?? null,
      fluencyScore: response.analytics?.fluency?.score ?? null,
      callSummary:
  ...
  useEffect(() => {
    ...
    for (const response of responses) {
      const sentiment =
        response.analytics?.user_sentiment || response.details?.call_analysis?.user_sentiment;
      ...
      const callCompletion = response.is_ended
        ? "Complete"
      ...
      const agentTaskCompletion = response.is_ended
        ? "Complete"
        ...
        <div className="bg-slate-200 rounded-2xl min-h-[120px] p-2 ">
          <div className="flex flex-row gap-2 justify-between items-center mx-2">
            <div className="flex flex-row gap-2 items-center">
              <p className="font-semibold my-2">Assessment Analysis</p>
            </div>
            <p className="text-sm">
              Tutor Screener used: <span className="font-medium">{interviewer?.name}</span>
            </p>
          </div>
          <p className="my-3 ml-2 text-sm">
            Assessment Description: <span className="font-medium">{interview?.description}</span>
          </p>
          <div className="flex flex-col gap-1 my-2 mt-4 mx-2 p-4 rounded-2xl bg-slate-50 shadow-md">
            <ScrollArea className="h-[250px]">
              {(() => {
                const hasTutorMetrics = tableData.some(
                  (row) => row.clarityScore !== null && row.clarityScore !== undefined,
                );
                return (
                  <DataTable
                    key={`${interview?.id}-${hasTutorMetrics ? "tutor" : "legacy"}`}
                    data={tableData}
          ...
          <div className="flex flex-row gap-1 my-2 justify-center">
            <div className="flex flex-col">
              <div className="flex flex-col gap-1 my-2 mt-4 mx-2 p-3 rounded-2xl bg-slate-50 shadow-md max-w-[400px]">
                <div className="flex flex-row items-center justify-center gap-1 font-semibold mb-1 text-[15px]">
                  ...
                  <InfoTooltip content="Average time candidates took to complete a teaching assessment" />
                </div>
                <div className="flex items-center justify-center">
                  <p className="text-2xl font-semibold text-indigo-600 w-fit p-1 px-2 bg-indigo-100 rounded-md">
                    {convertSecondstoMMSS(totalDuration / responses.length / 1000)}
                  </p>
                </div>
              </div>
              <div className="flex flex-col items-center justify-center gap-1 mx-2 p-3 rounded-2xl bg-slate-50 shadow-md max-w-[360px]">
                <div className="flex flex-row gap-1 font-semibold mb-1 text-[15px] mx-auto text-center">
                  ...
                  <InfoTooltip content="Percentage of teaching assessments completed successfully" />
                </div>
            ...
            <div className="flex flex-col gap-1 my-2 mt-4 mx-2 p-4 rounded-2xl bg-slate-50 shadow-md max-w-[360px]">
              <div className="flex flex-row gap-2 text-[15px] font-bold mb-3 mx-auto">
                <SmileIcon />
                Candidate Sentiment
                <InfoTooltip content="Distribution of candidate sentiments during teaching assessments" />
              </div>
              <PieChart
                sx={{
