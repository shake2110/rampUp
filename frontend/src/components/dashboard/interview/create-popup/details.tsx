...
function DetailsPopup({
  ...
  const onGenrateQuestions = async () => {
    ...
    try {
      ...
      // --- SMART MAPPER ---
      // AI models sometimes use different keys (questions, interview_questions, list, etc.)
      const rawQuestions =
        generatedQuestionsResponse.questions ||
        generatedQuestionsResponse.interview_questions ||
        generatedQuestionsResponse.items ||
        (Array.isArray(generatedQuestionsResponse) ? generatedQuestionsResponse : []);

      const interviewDescription =
        generatedQuestionsResponse.description || generatedQuestionsResponse.summary || "";

      // Final sanitization of the questions
      const updatedQuestions = (Array.isArray(rawQuestions) ? rawQuestions : []).map((q: any) => {
        const text =
          typeof q === "string" ? q : q.question || q.text || q.content || "New Question";
        return {
          id: uuidv4(),
          question: text.trim(),
          follow_up_count: 1,
      ...
      if (updatedQuestions.length === 0) {
        throw new Error("AI returned zero questions. Try a different model or prompt.");
      }
      ...
      setLoading(false); // Move to next step ONLY after data is set
    } catch (error) {
      console.error("Failed to process generated questions:", error);
      alert("AI response was malformed. Please try generating again or use the manual option.");
      setIsClicked(false);
      setLoading(false);
    }
  };
  ...
  const onResumeBased = () => {
    ...
    const updatedInterviewData = {
      ...
      name: name.trim(),
      objective: objective.trim(),
      questions: [], // No predefined questions
      interviewer_id: selectedInterviewer,
      question_count: 0,
      time_duration: String(duration),
      description: `This is a resume-based interview for the position: ${name.trim()}.`,
      is_anonymous: isAnonymous,
      ...
      <div className="text-center w-[38rem]">
        ...
        <div className="flex flex-col justify-center items-start mt-4 ml-10 mr-8">
          <div className="flex flex-row justify-center items-center">
            <h3 className="text-sm font-medium">Target Role / Interview Name:</h3>
            <input
              type="text"
              className="border-b-2 focus:outline-none border-gray-500 px-2 w-80 py-0.5 ml-3"
          ...
          <h3 className="text-sm font-medium">Interview Context / Objective:</h3>
          <Textarea
            value={objective}
            className="h-24 mt-2 border-2 border-gray-500 w-[33.2rem]"
            placeholder="e.g. You are testing their knowledge of Python APIs. Ask them to explain closures."
          ...
          <div className="flex-col mt-4 w-full">
            <div className="flex items-center cursor-pointer">
              <span className="text-sm font-medium">Make this a Resume-Based Interview?</span>
              <Switch
                checked={isResumeBased}
                className={`ml-4 mt-1 ${isResumeBased ? "bg-indigo-600" : "bg-[#E6E7EB]"}`}
                onCheckedChange={(checked) => setIsResumeBased(checked)}
              />
            </div>
            <span
              style={{ fontSize: "0.7rem", lineHeight: "0.66rem" }}
              className="font-light text-xs italic w-full text-left block"
            >
              Note: The candidate will be asked to upload their resume before starting.
            </span>
          </div>
          ...
          <div className="flex flex-row w-full justify-center items-center space-x-10 mt-5">
            <Button
              disabled={
                !(
                  name &&
                  objective &&
                  (isResumeBased || numQuestions) &&
                  duration &&
                  selectedInterviewer !== BigInt(0)
                ) || isClicked
              }
              className="bg-indigo-600 hover:bg-indigo-800 w-32 px-1 text-xs"
              onClick={() => {
                setIsClicked(true);
                if (isResumeBased) {
                  onResumeBased();
            ...
            >
              {isResumeBased ? "Create Interview" : "Generate Questions"}
            </Button>
            <Button
              disabled={
                !(
              ...
              className="bg-indigo-600 w-32 hover:bg-indigo-800 px-1 text-xs"
              onClick={() => {
                setIsClicked(true);
                onManual();
              }}
