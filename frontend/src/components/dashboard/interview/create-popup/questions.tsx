...
function QuestionsPopup({ interviewData, setProceed, setOpen }: Props) {
  ...
  const onSave = async () => {
    try {
      interviewData.user_id = user?.id || null;
      interviewData.organization_id = organizationId || null;
      ...
      const sanitizedInterviewData = {
        ...
        logo_url: "", // Supabase doesn't have a default org image like Clerk
      };
    ...
    <div>
      ...
      >
        ...
        <div className="my-3 text-left w-[96%] text-sm">
          {interviewData.is_resume_based
            ? "This is a resume-based interview. The candidate will upload their resume, and the interviewer will ask questions dynamically based on the resume and your objective."
