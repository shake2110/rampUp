    ...
    const fetchResponsesCount = async () => {
      ...
      try {
        const totalResponses =
          await ResponseService.getResponseCountByOrganizationId(organizationId);
        ...
        if (hasExceededLimit) {
          ...
          await ClientService.updateOrganization({ plan: "free_trial_over" }, organizationId);
        }
      } catch (error) {
  ...
  return (
    <main className="p-8 pt-0 ml-12 mr-auto rounded-md">
      <div className="flex flex-col items-left">
        <h2 className="mr-2 text-2xl font-semibold tracking-tight mt-8">My Assessments</h2>
            ...
            <Card className=" flex bg-gray-200 items-center border-dashed border-gray-700 border-2 hover:scale-105 ease-in-out duration-300 h-60 w-56 ml-1 mr-3 mt-4 rounded-xl shrink-0 overflow-hidden shadow-md">
              <CardContent className="flex items-center flex-col mx-auto">
                ...
                <CardTitle className="p-0 text-md text-center">
                  You cannot create any more assessments unless you upgrade
                </CardTitle>
              </CardContent>
            </Card>
          ) : (
              ...
              {isModalOpen && (
                <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
                  <div className="flex flex-col space-y-4">
                    ...
                    <p className="text-l text-center">
                      Contact <span className="font-semibold">founders@rampup.co</span> to upgrade
                      your plan.
                    </p>
                  </div>
                </Modal>
